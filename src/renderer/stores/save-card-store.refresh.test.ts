...
describe('save-card post-commit refresh', () => {
  ...
  it('invalidates stale unsaved work and replaces it with the forced fresh inventory', async () => {
    useSaveCardStore.getState().cacheInventory('ws-1', {
      bundles: [{ bundleId: 'obsolete-unsaved-package' }] as never,
      quotaWeakening: null,
    }, 123);
    const fresh = { bundles: [], quotaWeakening: null };
    ...
    expect(useSaveCardStore.getState().inventoryByWorkspace['ws-1'].inventory.bundles).toEqual([]);
