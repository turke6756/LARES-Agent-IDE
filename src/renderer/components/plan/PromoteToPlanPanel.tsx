...
export default function PromoteToPlanPanel({ workspace, proposalFilePath, onClose }: Props): React.ReactElement {
  ...
  }, [agents, query, workspace.id]);
  const selectedAgent: Agent | null = supervisors.find((agent) => agent.id === selectedId)
    ?? agents.find((agent) => agent.id === selectedId && agent.isSupervisor && agent.workspaceId === workspace.id)
    ?? null;
        ...
        <>
          ...
          <div className="mt-3 flex justify-end">
            <button
              ...
              disabled={dispatching || (selectedId !== NEW_SUPERVISOR_ID && !selectedAgent)}
