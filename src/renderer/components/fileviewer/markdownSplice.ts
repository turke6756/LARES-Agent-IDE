 ...
 * trailing newline(s) remark-stringify appends. List `spread` is
 * canonicalized first: Crepe re-serializes tight lists as loose (blank line
 * between items), and looseness alone must not make an untouched list look
 * changed — normText is strictly an alignment key, never emitted, so
 * unchanged-but-respread lists keep their original bytes. */
function normalizeBlock(lfSlice: string): string {
  const root = parseRoot(lfSlice);
  visit(root, (node) => {
    if (node.type === 'list' || node.type === 'listItem') {
      (node as { spread?: boolean | null }).spread = false;
    }
  });
  return stringifyRoot(root).replace(/\n+$/, '');
}
