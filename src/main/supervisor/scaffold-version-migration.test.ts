...
test('CF-1. supervisor CLAUDE.md: pristine v18 silently upgrades to v19 (no .bak); skill is written at v1', () => {
  ...
  try {
    ...
    assert.equal(readSidecar(workDir)['supervisor/CLAUDE.md'], 22, 'sidecar must record v19');
...
test('ML-S-1. supervisor CLAUDE.md: pristine v19 silently upgrades to v20 (no .bak)', () => {
  ...
  try {
    ...
    assert.equal(readSidecar(workDir)['supervisor/CLAUDE.md'], 22, 'sidecar must record v20');
  } finally {
    cleanup();
    rmrf(workDir);
  }
...
test('WP6-1. supervisor CLAUDE.md: pristine v17 silently upgrades to v18', () => {
  ...
  try {
    ...
    assert.equal(readSidecar(workDir)['supervisor/CLAUDE.md'], 22, 'sidecar must record v18');
  } finally {
    cleanup();
    rmrf(workDir);
  }
...
test('CP-0. precondition: the frozen v11 hash is registered for silent v11→v12 upgrade', () => {
  ...
  assert.equal(previous.version, 22, 'supervisor CLAUDE.md must be at version 21');
  assert.equal(
...
test('OV-2. supervisor CLAUDE.md: pristine v13 silently upgrades to v14', () => {
  ...
  try {
    ...
    assert.equal(readSidecar(workDir)['supervisor/CLAUDE.md'], 22, 'sidecar must record v17');
  } finally {
    cleanup();
    rmrf(workDir);
  }
