...
- **B3a** Provider-availability signal: add `availableProviders` (status +
  ...
  Hardened design (int_a7b3c9e2, GroupThink run 290646e4):
  [B3a availability-signal deliberation](deliberations/2026-08-05-b3a-availability-signal.md)
  — normative payload DTO + reason taxonomy, pure-resolver/acquisition split
  (process-global, no workspace coupling), evidence table with reason-specific
  clearing (agy auth-banner clears on confirmed send; grok free-usage-limit
  clears only screen-gated on Stop hook; grok classifier is evidence-gated and
  wholly omitted if the exhaustion artifact is unrecovered), claude
  mixed-window freshness rules, fallback order DEFERRED, exact v5 skill rule
  text, and the full test matrix.
<!--PLAN-INTEGRATION
{ "intent_id": "int_a7b3c9e2", "output_rel_path": "deliberations/2026-08-05-b3a-availability-signal.md",
  "changed": "hardened B3a to file:line standard: availableProviders DTO + reason order, static-floor governing principle, runtime-observation registry with reason-specific clearing, claude quota freshness rules, fallback-order deferred, v5 skill rule text, test matrix", "disposition": "active" }
-->
...
Decomposed into 10 bundle-contract WPs:
[supplements/2026-08-05-work-packages.md](supplements/2026-08-05-work-packages.md).
Git recovery point: local annotated tag
`plan-baseline/2026-08-05-provider-inclusive-groupthink-user-shapeable-pro-pigt5a83`
at commit 219adbaa (never pushed). Anything a WP deletes is one
`git show <tag>:<path>` away — no copy-aside archiving. Advisory: 71
uncommitted foreign working-tree files were not captured by the tag.
