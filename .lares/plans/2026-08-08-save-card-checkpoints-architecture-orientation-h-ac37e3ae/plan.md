...
**Deliberation output (int_7d41c9a2, folded 2026-08-09):**
[Intent-first Save, graduated concurrency, and per-activity worktrees — one architecture](deliberations/2026-08-09-intent-architecture-design.md)

**Packaged 2026-08-09:** work packages in
[supplements/2026-08-09-work-packages.md](supplements/2026-08-09-work-packages.md).
Baseline recovery point: local annotated tag
`plan-baseline/2026-08-08-save-card-checkpoints-architecture-orientation-h-ac37e3ae`
at commit `91c2e227b6a495e07b2d2e013dc3f7bfe06a498d` (local only, never pushed; deleted
code is recoverable via `git show <tag>:<path>`).

<!--PLAN-INTEGRATION
{ "intent_id": "int_7d41c9a2", "output_rel_path": "deliberations/2026-08-09-intent-architecture-design.md",
  "changed": "Concrete D5/D1/Q2 architecture: save_intents schema with trusted dispatch-time intent stamps; intent-assembler projection demoting topology to evidence; per-path blob concurrency classifier with exactly one blocking case and an evidence-bound three-gesture resolution picker; planning-activity worktrees (companion table, detached + refs/lares/activities/<run>/head, isolated three-way merge-back, proof-gated cleanup and startup recovery); candidate/commit contract v2 with readable messages; staged 7-WP buildable decomposition behind an intentPackaging flag", "disposition": "active" }
-->
