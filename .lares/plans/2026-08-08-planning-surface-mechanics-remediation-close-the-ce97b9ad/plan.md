...
> **Packaged 2026-08-08.** Work packages: [supplements/2026-08-08-work-packages.md](supplements/2026-08-08-work-packages.md)
> (18 WPs). Pre-implementation baseline:
> tag `plan-baseline/2026-08-08-planning-surface-mechanics-remediation-close-the-ce97b9ad`
> at commit `91c2e227b6a495e07b2d2e013dc3f7bfe06a498d` (local annotated tag, never
> pushed). Recovery framing: anything a package later deletes is one
> `git show <tag>:<path>` away — no copy-aside archiving.
...
**Cluster B deliberation RETURNED + reviewer-approved (2026-08-08):**
[Cluster B enforcement mechanism](deliberations/2026-08-08-int_b7d41c2e-cluster-b-enforcement.md)
(groupthink-serial, claude lead / codex reviewer). Core design: a five-layer honest
split — M1 (schema presence: required `reachability` block in a new
`PLAN-WORK-PACKAGES:v2` with grandfathered-v1 quarantine, mechanically enforced at
ingest), M2 (revert-refutation via an app-owned `prove_reachability` engine over a
path-built specimen tree, per-obligation mutation patches, stable failure markers,
evidence rows bound to the candidate tree OID), and S1–S3 (seam truth, construct
completeness/test honesty, worker reporting — human judgment via skill/checklist).
B4 is modelled as independent obligations: entry-seam links AND
production-constructs each independently refuted, which catches both the WP-6 and
mint incidents. Explicit recorded limitation: M2 is non-blocking until the D1
completion executor lands — F1/B4 remain partially remediated until then.
Packaging must cut Cluster B WPs from this document's §11 worklist, not from the
original B1–B4 sketches.

<!--PLAN-INTEGRATION
{ "intent_id": "int_b7d41c2e", "output_rel_path": "deliberations/2026-08-08-int_b7d41c2e-cluster-b-enforcement.md",
  "changed": "replaced B1-B4's open enforcement question with a five-layer design: mechanical ingest validation (v2 schema + quarantine), app-owned revert-refutation proof engine with code-state-bound evidence, and explicitly-human seam/completeness judgments; recorded that completion-blocking defers to D1", "disposition": "active" }
-->
...
**C2 research RETURNED (2026-08-08):**
[Provider sandbox capabilities — C2 research](research/2026-08-08-c2-provider-sandbox-capabilities.md)
(web-derived, untrusted-inbox origin, unverified claims flagged inside). Headline:
no provider offers usable Windows-native outbox enforcement today except Codex
(restricted token + synthetic SID + ACL, experimental); Claude Code hooks are
bypassed by any Bash write and agy sandboxing is explicitly unsupported on
Windows. The viable provider-neutral mechanism is a Tier-2 wrapper launcher using
the Codex restricted-token architecture (synthetic SID + write-allow ACEs on the
declared outbox, all four CLIs spawned under the write-restricted token), with
hook hardening as Tier 1. C2 packaging should target that layered design, not
per-provider hooks. Note: Windows Sandbox (Tier 3) is unavailable on this host
(Windows 11 Home).

<!--PLAN-INTEGRATION
{ "intent_id": "int_c2a90f13", "output_rel_path": "research/2026-08-08-c2-provider-sandbox-capabilities.md",
  "changed": "replaced C2's unspecified 'enforce at the host/tool boundary' goal with a concrete layered mechanism: OS-level restricted-token wrapper launcher (provider-neutral), hooks demoted to defense-in-depth", "disposition": "active" }
-->
...
**D3 open question RESOLVED (2026-08-08):** the returned forensic investigation
[D3 continuation-turn forensics](research/2026-08-08-d3-continuation-turn-forensics.md)
establishes it is a **recorder bug**, not concurrency semantics: the coordinator
conflates "prior row still in `openByAgent`" with "prior model turn active"
(`turn-coordinator.ts:203-216`), an identity-unsafe `clearAgent` orphans the new
handoff turn (`turn-coordinator.ts:449-453`), and startup reconciliation only
examines the newest 50 turns, which is why row 1774 alone stayed `open`. The row
overlap is real at the checkpoint-close level (0.5–1.2s) but not at the agent-turn
level. The report specifies what a correct fix records (turn-identity
compare-and-delete cleanup, serialize-behind-close, unbounded reconciliation, and
the `brick_saved`/`successor_started`/`successor_oriented` attempt results keyed to
`handoff_attempt_id`) — D1's schema design must take these as input.

<!--PLAN-INTEGRATION
{ "intent_id": "int_d3f8266b", "output_rel_path": "research/2026-08-08-d3-continuation-turn-forensics.md",
  "changed": "resolved D3's open question as recorder bug (state-ownership race + newest-50 reconciliation gap), replacing the semantics-vs-bug fork with a concrete fix specification that feeds the D1+D4 deliberation", "disposition": "active" }
-->
...
**D1+D4 deliberation RETURNED — two-planner convergence (2026-08-08):**
[D1+D4 ledger schema and versioning policy](deliberations/2026-08-08-int_d1d47a05-ledger-and-versioning.md)
(groupthink-parallel run, claude + codex planners, converged after two exchange
rounds). Decision of record: SQLite owns mutable execution facts, git owns
portable plan artifacts; strengthen-and-join the eight existing tables rather than
build a monolith; **four** new tables (`plan_package_gate_attempts`,
`plan_package_gate_commit_links`, `plan_package_deployment_events`,
`continuation_handoff_result_events`) plus narrow guarded `ALTER`s; one
main-process transition service (`package-ledger.ts`) as the sole
`plan_work_packages.state` writer, self-witnessing evidence, with a source-guard
test; D3's handoff results recorded as a separate lifecycle that never touches
checkpoint-turn status. D4 resolved: **track every durable plan-folder file
including `plan.json`** (identity/responsibility only — never execution state),
enforce via anchored ignores for lock/temp siblings + a checker script + a single
reviewed migration commit for the surveyed population (8 folders, 47 untracked
files, all `plan.json` untracked). Acceptance is DB-only rendering compared
against the verified 13-commit chain. Packaging must cut Cluster D WPs from this
document's §8 sequenced worklist.

<!--PLAN-INTEGRATION
{ "intent_id": "int_d1d47a05", "output_rel_path": "deliberations/2026-08-08-int_d1d47a05-ledger-and-versioning.md",
  "changed": "settled the D1 ledger as strengthen-and-join (4 new tables + one transition service, sole state writer) and D4 as track-everything-durable including plan.json, with enforcement checker and reviewed population migration; D3 results get a separate handoff lifecycle", "disposition": "active" }
-->
