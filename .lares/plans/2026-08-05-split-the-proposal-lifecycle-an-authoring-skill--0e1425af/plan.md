...
**Deliberation returned and folded in:** see
[deliberations/2026-08-05-promotion-prompt-design.md](deliberations/2026-08-05-promotion-prompt-design.md)
— resolves open questions 1–2 (template constant `PROPOSAL_PROMOTION_PROMPT_TEMPLATE` in
`src/shared/constants.ts`; keep the mounted picker, default to the structural workspace
supervisor via `getSupervisor`), requires a validated `artifact_id` at both dispatch
boundaries, records the prop_e0001372 C1 dependency boundary, and pre-cuts executable
work packages WP1–WP7.

<!--PLAN-INTEGRATION
{ "intent_id": "int_6781b552", "output_rel_path": "deliberations/2026-08-05-promotion-prompt-design.md",
  "changed": "Resolved OQ1 (prompt lives as PROPOSAL_PROMOTION_PROMPT_TEMPLATE constant in src/shared/constants.ts, no scaffold bump for it), OQ2 (keep mounted picker, default = structural workspace supervisor via getSupervisor with race-guarded async resolution), added required artifact_id validation at panel + dispatch boundaries, recorded C1 (int_9e5d0c47) dependency boundary, and produced concrete WP1-WP7 edits with tests", "disposition": "active" }
-->
...
## Work packages (packaged 2026-08-05, bundle-contract shape)

Baseline: local annotated tag `plan-baseline/2026-08-05-split-the-proposal-lifecycle-an-authoring-skill--0e1425af`
at commit `e46ae812` (tag object 5e0bc8d4). Recovery framing: anything a WP deletes is one
`git show <tag>:<path>` away — no copy-aside archiving. Advisory at tagging time: 49 uncommitted
paths in the worktree are NOT captured by the tag (other workstreams' in-flight edits; non-blocking).

### WP-1 — Promote-gesture prompt wire (renderer + shared constant)
- **Files:** `src/shared/constants.ts`, `src/renderer/components/plan/promotion-dispatch.ts`,
  `src/renderer/components/plan/PromoteToPlanPanel.tsx`, `src/renderer/components/plan/ProposalCardGallery.tsx`,
  `src/renderer/components/plan/proposal-card-metadata.ts`, plus their sibling `*.test.ts(x)`.
- **Dep:** plan_e0001372 **WP-A must land and gate first** — WP-1 adds a constant to
  `src/shared/constants.ts`, the same file WP-A rewrites while re-deriving pristine scaffold hashes.
  WP-1 is NOT renderer-only; do not run it parallel to WP-A. Also sequence after their WP-J on the
  gallery/metadata files (chain: WP-J → WP-1 → WP-5 → their WP-F). No scaffold bump here: this
  constant is renderer-bundle and is not provisioned into `PROPOSAL_TO_PLAN_TREE`. C1/saga work
  explicitly out of scope.
- **Do:** implement the deliberation's WP1–WP7 exactly as specified in
  [deliberations/2026-08-05-promotion-prompt-design.md](deliberations/2026-08-05-promotion-prompt-design.md)
  §"Concrete edits": `PROPOSAL_PROMOTION_PROMPT_TEMPLATE` constant; `isValidProposalArtifactId`
  shared validator enforced at panel (disable + error) and dispatch (throw before side effects);
  `artifactId` on card metadata, threaded via gallery to panel; picker default = structural workspace
  supervisor (`getSupervisor`, `hasUserSelectedRef` race guard, `resolvedDefault` merged visibly,
  terminal→revive); full test list from the doc.
- **Accept:** new instruction text contains `Do NOT run capture`, the artifact_id line, and the
  `scope -> promote -> deliberate -> integrate -> package` ordering; old "capture/scope/promote as
  applicable" wording asserted absent; invalid id → no side effects; default-resolution race tests
  green. NO scaffold-version bump for this constant (renderer-bundle, not provisioned).
- **Non-goals:** no saga deletion or pending-request migration (prop_e0001372 int_9e5d0c47 owns it);
  no state-dir editable template; no skill-file edits.
- **Cross-plan constraint (boundary B, see ARC.md ## Decisions):** `proposalRelPath` is expected to
  become **server-supplied** by plan_e0001372's WP-F preflight. Do NOT harden, cache, or add tests
  around any renderer-side path derivation — WP-F tears that out. Build the template text and the
  panel-side `isValidProposalArtifactId` sanitizer only; the renderer artifact id is destined to
  become WP-F's `artifactIdCrossCheck?` input, never identity authority. The template TEXT and the
  three text assertions above survive WP-F by contract.
- **Verify:** `npm run build`; `npm run test:renderer` (iterate via
  `npx vitest run --config vitest.config.ts src/renderer/components/plan`).

### WP-2 — `write-proposal` workspace-shared skill (absorbs capture)
- **Files:** scaffold skill-content surface in `src/shared/constants.ts` (or wherever provisioned
  skill bodies live — worker locates the provisioning constants), skill body for
  `write-proposal/SKILL.md` provisioned to ALL lanes (supervisor, worker claude+codex, researcher);
  `proposal-to-plan` dispatcher SKILL.md (capture row removal rides WP-4's edit — coordinate).
- **Dep:** shares ONE scaffold-version bump with WP-3/WP-4 (land together or sequence; per
  scaffold-content rules deployment to existing workspaces is not automatic). Lands AFTER
  plan_e0001372's WP-A, gated — see ARC.md ## Decisions.
- **`previousHashes` is CUMULATIVE — additive only (boundary G1, mechanical).** `PROPOSAL_TO_PLAN_TREE`
  (`src/main/supervisor/index.ts:1272-1298`) is **per-file** versioned; a "shared bump" here means
  several files each advancing their own version, not one global revision. When you bump a file,
  **ADD the new hash entry to the existing `previousHashes` map and preserve every prior entry,
  including entry `1`.** Never replace the map with a single freshly-derived hash.
  Why (verified in `src/main/scaffold-writer.ts`): the update path looks up
  `previousHashes?.[diskVersion] ?? previousHashes?.[1]` (:186) and the retirement path scans
  `Object.values(previousHashes)` (:132). Drop the old entries and a workspace sitting on the prior
  version stops matching a known managed body — it is **not** stranded (it still gets backed up and
  upgraded at :200-207), but every existing workspace acquires a spurious `.bak.<ts>` plus a
  "differed from known managed content" warning, a genuine user edit becomes indistinguishable from
  a pristine copy, and `pruneScaffoldBackups` never reclaims those backups because they no longer
  match a known shipped body (`SCAFFOLD_BAK_RETENTION` doc, :49-54). It fails only on a real
  workspace after restart — the test suites do not catch it.
- **Do:** per Part 1a + Part 2 amendment: skill owns threshold (proposal vs remember vs lesson vs
  supporting), path rule (subscription-gated `supporting/`, default `.lares/proposals/YYYY-MM-DD-<slug>.md`),
  stamp contract (`artifact_id` prop_+8hex w/ collision scan; REQUIRED `author` = specific launch
  title, `author_agent_id`, `author_role`, `authored_at`; `author_provider` encouraged; generic role
  label in `author` fails), required "In plain terms" lead (what / why / what changes for the user);
  hand-off sentence = "tell the human; lifecycle continues only from the Plans pane".
- **Accept:** skill provisioned to all lanes on migration; capture absorbed (its playbook content
  lives here); contract text matches Part 2's frontmatter block verbatim.
- **Non-goals:** no card-rendering changes (WP-5); no promotion-prompt changes (WP-1).
- **Verify:** scaffold/version-migration tests + full main suite green.

### WP-3 — `read-planning-surface` workspace-shared skill (orient split, read side)
- **Files:** new skill body provisioned to all lanes; same scaffold surface as WP-2.
- **Dep:** WP-4 (the orient split's write side) — same scaffold bump; design source is Part 1c.
  G1's cumulative-`previousHashes` rule applies to any file this WP bumps.
- **Do:** surface-wide read+interpret skill: whole-surface state report (flat proposals incl.
  scope-verdict/PLAN-INTENT markup, plan folders, the promoted-but-bare-card gap), the rung
  derivation + decision-table REPORTING from orient, and the interpretation rules (bare proposal is
  terminal-valid; `ran` unavailable pre-ledger, never inferred; witnessed activity = whether to look,
  not quality; supporting/ is subordinate; frontmatter authorship is self-claim vs witnessed truth).
- **Accept:** boundary contract explicit in the skill: never writes, never launches, never appends
  `assigned` events; output = state report + safe-next-actions routed to responsible supervisor/human;
  may recommend "run orient on plan X" without running it.
- **G4 boundary — what this skill does NOT get.** The **responsibility determination** stays in
  `proposal-to-plan`'s `orient` (WP-4): it gates a mutation, and this skill cannot write. This skill
  may *report* who is currently responsible as observed state; it never renders the verdict that
  authorizes a supervisor to act.
- **Cite, do not restate.** Where this skill describes rung/responsibility derivation, it **cites**
  the normative anchor `references/contracts/responsibility.md` §`Determination` rather
  than duplicating the rules. Two copies of the derivation rules in two separately-versioned skill
  bodies will drift; that drift is the long-term failure mode of the split.
- **Non-goals:** no ARC-META refresh here (stays responsible-supervisor-only in the method library).
- **Scope bound (boundary M5, see ARC.md ## Decisions):** document **disk-derived state only**.
  Explicitly disclaim — do not describe — gallery grouping/collapse, DB projection of work packages
  or responsibility, and readiness gates. Those surfaces belong to plan_e0001372 (WP-J/C/E/H) and are
  changing through its wave 4; documenting them here ships stale text. That documentation is deferred
  to plan_e0001372 as a named open item, scheduled after their WP-Z gates.
- **Verify:** scaffold tests + main suite; manual skill-invocation smoke from a worker lane.

### WP-4 — `proposal-to-plan` trigger rewrite (promotion-entry only)
- **Files:** `proposal-to-plan` SKILL.md (dispatcher) + orient playbook; same scaffold surface/bump
  as WP-2/3.
- **Do:** rewrite trigger description to promotion-prompt-entry only (no "use whenever you author"
  language); remove the capture mode row from SKILL.md's mode table (content moved by WP-2) and
  **retire** the deployed activity file per G2 below; orient keeps ARC-META refresh AND the
  responsibility determination as responsible-supervisor method-library steps, while only its
  cross-surface *reporting* half is cited as moved to `read-planning-surface` (see G4); method
  library otherwise untouched (`references/contracts/`, `scripts/`).
- **G1 — `previousHashes` additive:** the cumulative-map rule in WP-2's Dep block applies verbatim to
  every file this WP bumps (`SKILL.md`, `references/activities/orient.md`). Preserve entry `1`.
- **G2 — capture.md is RETIRED, not row-deleted (mechanical).** Do **not** remove the
  `references/activities/capture.md` entry from `PROPOSAL_TO_PLAN_TREE`. Set `removed: true`,
  `content: ''`, bump `version` to 4, and **KEEP its `previousHashes`** — extend `{1, 2}` with the
  current v3 body hash so both pristine and user-modified deployed copies retire cleanly (backup then
  delete). Deleting the row outright means the scaffold writer never touches the file again: the
  deployed `references/activities/capture.md` stays on disk in every existing workspace forever,
  orphaned and still openable, while the rewritten SKILL.md no longer references it. The
  `ScaffoldFile.removed` doc comment (`src/main/scaffold-writer.ts:30-38`) states this requirement
  outright; `src/main/supervisor/index.ts:3301` (`orchestration-spike/SKILL.md`) is a landed
  precedent of the exact shape.
- **G3 — preserve WP-A's A2 continuity rule (contested file).** `SKILL.md` is the **only**
  byte-contested file between the two plans, and at this WP's start point it already carries
  plan_e0001372 WP-A's A2 continuity rule (no "phase done, continue?" pauses between
  scope→promote→deliberate→integrate→package; the single built-in stop is after `package`). The
  trigger/mode-table rewrite must carry A2 through **verbatim** — ae889b24 is writing it as a
  self-contained block for exactly this reason. Assert its survival in a test.
- **G4 — responsibility determination STAYS in this skill.** `orient.md` keeps the
  responsibility-derivation step (last `assigned` event wins → may this supervisor mutate?) under a
  **stable anchor heading** so other playbooks can cite it by name instead of saying "run orient".
  It is a precondition of a WRITE action, not a surface report: plan_e0001372 WP-A's matching-EEXIST
  loser rule in `promote.md` depends on it, and a skill contractually forbidden from writing cannot
  gate a mutation. Only the cross-surface state *reporting* moves to `read-planning-surface`.
  **Anchor (settled): `references/contracts/responsibility.md` §`Determination`.** Per G6, WP-A
  CREATES that contract file at v1 (lifting the rules `orient.md` already carries) so its `promote.md`
  citation is never dangling in a deployed workspace; this WP then removes the duplicated rules from
  `orient.md`, cites the contract, and owns its content from v2 onward. `orient.md` is **not**
  byte-edited by WP-A — this WP owns that file outright.
- **Accept:** no agent-facing authoring trigger survives; injected-prompt entry documented; lane
  rules preserved; A2 present verbatim; capture.md entry present with `removed: true` and an intact
  `previousHashes`; the responsibility-determination anchor exists and `promote.md` resolves to it.
- **Non-goals:** no playbook/contract content changes beyond the split citations.
- **Verify:** scaffold tests + main suite.

### WP-5 — Authorship on proposal cards (two registers, specific identity)
- **Files:** `src/renderer/components/plan/proposal-card-metadata.ts` + card render component(s),
  `src/main/**/proposals-watcher.ts` (+ its projection/DB surface as discovered), tests.
- **Dep:** sequence after WP-1 on the shared file (chain: their WP-J → WP-1 → WP-5 → their WP-F).
  After WP-J, promoted proposals render in a **collapsed promoted group** carrying full card
  metadata — the byline must render inside that group too, not only in the active gallery.
- **Do:** per Part 2 amendment: byline renders `<agent title> · <lane> · <uuid first 8> · <date>`
  from self-declared frontmatter; witnessed register resolves its stored agent id to title + short id
  (never a bare role word); mismatch marker (e.g. "byline unwitnessed") when self-declared
  `author_agent_id` disagrees with the witnessed writer; hover/expand may show full uuid + provider.
- **Accept:** both registers displayed, labeled honestly, never merged; disagreement flagged;
  anonymous pre-rule cards degrade gracefully (no crash, show what exists).
- **Non-goals:** no enforcement of frontmatter on ingest (authoring enforcement is WP-2's skill);
  no backfill (WP-6).
- **Verify:** renderer suite + main suite for watcher changes.

### WP-6 — Backfill pre-rule proposals (optional, cheap)
- **Files:** the handful of pre-rule files in `.lares/proposals/` (e.g. `2026-07-30-save-card-commit-ui.md`).
- **Dep:** after WP-5 (so the display exists to check against).
- **Do:** add specific `author` + `author_agent_id` lines from known history (ids recoverable from
  the agents table); no tooling.
- **Accept:** previously-anonymous cards render specific bylines; `amended_at` stamped.
- **Non-goals:** no speculative attribution — leave unknown authors unattributed.
- **Verify:** visual check on the proposals pane — **look inside the collapsed promoted group**, not
  only the active gallery: after plan_e0001372's WP-J, several backfill targets are promoted and
  therefore collapsed. A backfilled byline that is merely hidden is not a failed backfill.
- **Cross-plan constraint (boundary D):** do NOT add `promoted_to` / `promoted_at` here. The
  promote-time stamp and the one-time backfill of the 3 unstamped promoted proposals are
  plan_e0001372's WP-A/WP-J. This WP touches authoring-time keys only.
